---
name: ci-doctor
description: Analyze GitHub Actions failures and identify root causes. Run with a workflow run URL, or `help` for a short explanation.
argument-hint: <url> | help
---

Arguments: $ARGUMENTS

If `$ARGUMENTS` is exactly `help` (or empty), do NOT start the analysis. Instead print the following short explanation and stop:

> **ci-doctor** — Analyze a failed GitHub Actions workflow run and identify its root cause.
>
> **Usage:** `/ci-doctor <workflow run URL>`
>
> **What it does:**
> 1. Identifies which job/commit failed and what specifically caused the exit code 1.
> 2. Checks the last 10–20 runs of the same job to assess flakiness (one-time vs recurring).
> 3. If recurring, finds and verifies the breaking commit.
> 4. Determines the root cause from logs and history.
> 5. Checks for an existing open PR that already fixes the issue.
> 6. Writes a final report: summary, flakiness, breaking commit, root cause, fix PR, recommendation.

Otherwise, treat `$ARGUMENTS` as a GitHub Actions URL and investigate it.
